...
2. **GitHub MCP definido, mas nenhuma sessão autenticada.** O `.mcp.json` aponta para o servidor remoto oficial (`https://api.githubcopilot.com/mcp/`), mas cada integrante ainda precisa aprovar o servidor e autenticar via `/mcp`. Enquanto isso não ocorrer, os agentes não operam issues nem PRs, e o backlog exigido pela seção 9 do CLAUDE.md é mantido manualmente. **Não verificado:** se a organização `labsitio` permite esse endpoint. Plano B em caso de bloqueio: servidor local via Docker com PAT.
